...
try {
    ...
    $history = array_map(function($item) {
        // Thumbnail path logic (relative to frontend)
        $t = $item['thumbnail_path'];
        if ($t && strpos($t, 'http') !== 0) {
            if (strpos($t, '..') !== 0) {
                // assume 'uploads/...' -> '../uploads/...'
                $t = '../' . ltrim($t, '/');
            }
        }
    ...
    // http_response_code(500); // Keep 200 so frontend can parse JSON error
