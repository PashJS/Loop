...
try {
    ...
    $stmt = $pdo->prepare("DELETE FROM email_verifications WHERE user_id = ? AND verification_type = 'password_change'");
    $stmt->execute([$userId]);
    ...
    ");
    $stmt->execute([$userId, $hashedPassword, $token, $expiresAt]);
